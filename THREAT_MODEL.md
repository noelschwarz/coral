...
| T4 | Compromised extension exfiltrates sessions | Least privilege + user education + CORS allowlist | **Partially implemented** — CORS regex restricts the daemon to `chrome-extension://*` origins; the daemon never returns `state_blob` over HTTP (sessions are restored only into Playwright contexts the daemon owns, ADR-010). Extension permissions audit pending. |
| T5 | Agent exceeds policy | Daemon-side enforcement via Playwright routes | **Partially implemented** — every session gets a route handler that audits each navigation as `navigation`. Policy logic (allowed/denied paths, action verbs, rate limits) lands in week 3; the hook point is in place. |

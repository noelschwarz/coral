...
| T6 | Malicious agent with CDP control exfiltrates | **_Accepted risk / agent trust boundary — document clearly_** | Documented in spec; per-session Chromium (ADR-010) bounds the blast radius to that one session. **As of Track N (ADR-018)**, session close optionally writes cookie deltas back into the vault, gated by the policy's `allowed_paths` — agents now have a write path into vault state, but only for cookies whose scope the operator already trusted. Worst case: an agent could poison its own session's cookie to lock the user out; recovery is to re-capture from the user's main Chrome via the extension. |
...
| T10 | Stale session replay | Document limitation + kill-on-login policies + write-back | **Partially implemented.** Per-pack `session.kill_on_redirect_to_login: true` (default in all six bundled packs) detects when the site forces re-auth and tears the session down. **Track N (ADR-018)** adds policy-gated cookie write-back on session close: in-flight `Set-Cookie` updates from auth-token refresh or server session bumps now survive to the next session-open. Recapture-on-401 is the next layer (planned, PR N3). Cookie binding (Device-Bound Session Credentials, CHIPS) is the longer-horizon answer — out of scope for v1. |
